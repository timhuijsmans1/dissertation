...
from quapy.method.aggregative import SVMNKLD
...
def prevalence_calculator(model, sparse_matrix):
    ...
    predicted_instances = model.classify(instance_csr_matrix) # predict labels
    prevalences = prevalence_from_labels(predicted_instances)
    return prevalences, predicted_instances
...
def main(
    ...
    for ticker in ticker_list:
        ...
        for i, day in enumerate(daily_intervals):
            ...
            # split instances on pre-market and intra-market by their timestamp 
            # below code needs to be duplicated for both vocabularies

            print("writing instances")
            # turn the engineered features into instances and write instances
            prediction_day_path = os.path.join(output_paths['predictions'], daily_file_name)
            vocabulary_union = load_vocabulary(union_vocabulary_path)[1:] # skip the first token for obtaining the right shape, as from_sparse removes one dimension of the vectors
            vocabulary_emoticon = load_vocabulary(emoticon_vocabulary_path)[1:] 
            union_model = model_loader(union_model_path)
            emoticon_model = model_loader(emoticon_model_path)
            sparse_total_matrix_union = build_sparse_matrix(engineered_day_path, vocabulary_union).T
            sparse_total_matrix_emoticon = build_sparse_matrix(engineered_day_path, vocabulary_emoticon).T
...
if __name__ == "__main__":
    ...
    UNION_VOCABULARY_PATH = '../../data/vocabularies/train_union_vocabulary.txt'
    EMOTICON_VOCABULARY_PATH = '../../data/vocabularies/train_emoticon_vocabulary.txt'
    UNION_MODEL_PATH = "./SVMRAE_union.pkl"
    EMOTICON_MODEL_PATH = "./SVMRAE_emoticon.pkl"
    TICKER_LIST = ["$TSLA"]
